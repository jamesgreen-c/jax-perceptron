...
import jax
from core.models.one_versus_one import ovo_perceptron, ovo_evaluation
import numpy as np
...
def cross_val_step(_carry, param_index):
    ...
    weights, _ = ovo_perceptron(train_X_a, train_Y_a, d_param_set, 8)

    # evaluate the perceptron
    test_X = jnp.array([test_X_fold])
    test_Y = jnp.array([test_Y_fold])
    test_error, _ = ovo_evaluation(weights, train_X_a, test_X, test_Y, d_param_set)
...
def run_wrapper(
    ...
    weights, _ = ovo_perceptron(train_X, train_Y, d_param_set, 8)
